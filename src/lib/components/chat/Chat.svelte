<script lang="ts">
	...
	const sendPromptSocket = async (model, responseMessageId, _chatId) => {
		...
		files.push(
			...(userMessage?.files ?? []).filter((file) => file.type === 'image'),
			...(responseMessage?.files ?? []).filter((file) => file.type === 'image')
		);
		// Remove duplicates
		files = files.filter(
			(item, index, array) =>
			...
			<Pane defaultSize={50} class="h-full flex w-full relative">
				...
				<div class="flex flex-col flex-auto z-10 w-full">
					{#if $settings?.landingPageMode === 'chat' || createMessagesList(history.currentId).length > 0}
						...
						<div class="pb-[2.5rem]"> <!-- Increased padding from 1rem to 2.5rem -->
							<MessageInput
								{history}
								{selectedModels}
								bind:files
							...
							<!-- Show disclaimer only in chat mode -->
							<div class="absolute bottom-4 left-0 right-0 text-xs text-gray-500 text-center w-full px-2">
								Sora can make mistakes. Verify important information.
							</div>
						</div>
					{:else}
