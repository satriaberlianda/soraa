import { WEBUI_API_BASE_URL, EXTERNAL_AUTH_API } from '$lib/constants';

const sendToExternalAuth = async (userData: {
    name: string;
    phone_number: string;
    email: string;
    password: string;
    ...
    const res = await fetch(`${EXTERNAL_AUTH_API}/x/users`, {
        method: 'POST',
        headers: {
            'Intellence-Key': 'micyna',
    ...
    if (!res.ok) {
        ...
        throw responseData.message || responseData.error || 'Failed to register';
    }
